prompts = ['''
    ...
    Example 2: Give me the gross profit on order number 10101, response:- SELECT order_number, buyer_price, supplier_price, system_freight_cost, (buyer_price - supplier_price - CAST(system_freight_cost AS DECIMAL) / 1000) as Gross_Profit FROM order_table where order_number = '10101';
    Example 3: (for finance view) give me order details of order number 10544, response:- SELECT order_number, dispatch_date, product_name, single_quantity, supplier_name, supplier_gst, supplier_price, supplier_credit_note_value, buyer_name, buyer_gst, buyer_price, buyer_credit_note_value, buyer_payment_terms, buyer_delivery_terms, transporter_name, vehicle_number, lr_number, freight_cost, freight_payment_status, godown_name , loading_address,delivered_to_with_parent, delivery_address, company_gst, purchase_order_number, order_status FROM order_table where order_number = '10544';
    Example 4: (for vehicle transit details - vehicle is in transit if its order_status is VEHICLE DISPATCHED or VEHICLE REACHED AT DESTINATION) give me vehicle transit details of orders, response:- SELECT order_number , godown_name, destination_parent_name, order_status, eway_bill_expiry_date, dispatch_date, expected_delivery_date FROM order_table where order_status = 'VEHICLE REACHED AT DESTINATION' or order_status = 'VEHICLE DISPATCHED';
